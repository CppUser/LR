...
class ULRRangeWeaponInstance;

UENUM(BlueprintType)
enum class EAbilityTargetingSource : uint8
{
...
class LR_API ULRGA_RangeWeapon : public ULRGA_FromEquipment
{
	GENERATED_BODY()
public:
	ULRGA_RangeWeapon(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintCallable, Category="LR|Ability")
	ULRRangeWeaponInstance* GetWeaponInstance() const;
...
protected:
	struct FRangedWeaponFiringInput
	{
		...
		ULRRangeWeaponInstance* WeaponData = nullptr;
...
protected:
	...
	FHitResult DoSingleProjectileTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHits) const;

	// Traces all of the bullets in a single cartridge
	void TraceProjectilesInCartridge(const FRangedWeaponFiringInput& InputData, OUT TArray<FHitResult>& OutHits);
	...
	FTransform GetTargetingTransform(APawn* SourcePawn, EAbilityTargetingSource Source) const;
