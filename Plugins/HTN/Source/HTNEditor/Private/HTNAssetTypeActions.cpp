...
FText FHTNAssetTypeActions::GetName() const
{
	return LOCTEXT("HTNAssetName", "Hierarchical Task Network");
}

FColor FHTNAssetTypeActions::GetTypeColor() const
{
	return FColor(182, 61, 14);
}

UClass* FHTNAssetTypeActions::GetSupportedClass() const
{
	return UHTN::StaticClass();
}

uint32 FHTNAssetTypeActions::GetCategories()
{
	return IAssetTools::Get().FindAdvancedAssetCategory("Artificial Intelligence");
}

void FHTNAssetTypeActions::OpenAssetEditor(const TArray<UObject*>& InObjects,
...
void FHTNAssetTypeActions::GetActions(const TArray<UObject*>& InObjects, FMenuBuilder& MenuBuilder)
{
